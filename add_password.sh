...
def create_camera():
    ...
    if 'id' in data and data['id']:
        ...
        if cam_id in cameras:
            return jsonify({'success': False, 'error': f'Камера с ID {cam_id} уже существует'}), 400
    else:
        used_ids = set(int(cam_id) for cam_id in cameras.keys() if cam_id.isdigit())
        new_id = 1
        while new_id in used_ids:
            new_id += 1
        cam_id = str(new_id)
        if new_id > camera_counter:
            camera_counter = new_id
...
@app.route('/api/cameras/<cam_id>/video', methods=['PUT'])
@auth.login_required
def change_camera_video(cam_id):
    ...
    if not new_video:
        return jsonify({'success': False, 'error': 'Не указан видеофайл'}), 400
    ...
    cameras[cam_id]['video'] = new_video
